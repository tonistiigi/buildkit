...
func addItemToStorage(k *cacheKeyStorage, it *item) (*itemWithOutgoingLinks, error) {
	...
	var id string
	if len(it.links) == 0 {
		id = digest.FromBytes([]byte(fmt.Sprintf("%s@%d", it.dgst, it.output))).String()
	} else {
		id = identity.NewID()
	}
	...
	for i, m := range it.links {
		for l := range m {
			...
			cl := solver.CacheInfoLink{
				Input:    solver.Index(i),
				Output:   solver.Index(it.output),
				Digest:   it.dgst,
				Selector: digest.Digest(l.selector),
			}
	...
	itl := &itemWithOutgoingLinks{
		item:  it,
		links: map[solver.CacheInfoLink][]string{},
	}
