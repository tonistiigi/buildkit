...
func normalizeItem(it *item, state *normalizeState) (*item, error) {
	...
	if len(it.links) == 0 {
		id := digest.FromBytes([]byte(fmt.Sprintf("%s@%d", it.dgst, it.output)))
	...
	for i, m := range it.links {
		...
		for l := range m {
			nl := nlink{dgst: it.dgst, output: it.output, input: i, selector: l.selector}
