...
func (c *Client) solve(ctx context.Context, def *llb.Definition, runGateway runGatewayCB, opt SolveOpt, statusChan chan *SolveStatus) (*SolveResponse, error) {
	...
	eg.Go(func() error {
		...
		res = &SolveResponse{
			...
			CacheExporterResponse: resp.CacheExporterResponse,
		}
		return nil
	})
	...
	// FIXME(AkihiroSuda): dedupe const definition of cache/remotecache.ExporterResponseManifestDesc = "manifest"
	if manifestDescJSON := res.CacheExporterResponse["manifest"]; manifestDescJSON != "" {
...
func parseCacheOptions(opt SolveOpt) (*cacheOptions, error) {
	...
	res := cacheOptions{
		options: controlapi.CacheOptions{
			// old API (for registry caches)
			ExportRef:   legacyExportRef,
			ExportAttrs: legacyExportAttrs,
			ImportRefs:  legacyImportRefs,
			// new API
