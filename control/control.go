...
func translateLegacySolveRequest(req *controlapi.SolveRequest) error {
	...
	if legacyExportRef := req.Cache.ExportRef; legacyExportRef != "" {
		ex := &controlapi.CacheOptionsEntry{
			Type:  "registry",
			Attrs: req.Cache.ExportAttrs,
		}
		if ex.Attrs == nil {
		...
		req.Cache.ExportRef = ""
		req.Cache.ExportAttrs = nil
	}
	// translates ImportRefs to new Imports (v0.4.0)
	for _, legacyImportRef := range req.Cache.ImportRefs {
