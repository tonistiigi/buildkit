...
func (t *testExporterTarget) Add(dgst digest.Digest, output int) ExporterRecord {
...
type testExporterRecord struct {
	...
	results int
	links   int
	linkMap map[*testExporterRecord]struct{}
}
...
func (r *testExporterRecord) LinkFrom(src ExporterRecord, index int, selector string) {
	if s, ok := src.(*testExporterRecord); ok {
		if r.linkMap == nil {
			r.linkMap = map[*testExporterRecord]struct{}{}
		}
		r.linkMap[s] = struct{}{}
		r.links++
	}
